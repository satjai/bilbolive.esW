...
class Firmador{
	const POLITICA_FIRMA = array(
		"name"	 => "Política de firma para facturas electrónicas TicketBAI",
		"url"	 => "https://ticketbai.eus/politicafirma/",
	...
	// Funcion para obtener la key y el cert a partir de un certificado p12
	public function firmar($certificadop12, $clavecertificado, $xmlsinfirma) {
		// Comprobamos que el archivo de certificado sea valido
		...
		// Obtenemos el cert y la key del certificado
		...
		// Generamos la firma digital a partir de la informacion del certificado
		...
		$xml1					= $xmlsinfirma;
		// Anadimos la firma al documento XML
	...
	public function insertaFirma($xml) {
		...
		// Usar esta instruccion si el XML esta contenido en una cadena de texto
		//$d->loadXML($xml);
		// Sin embargo, si lo que estamos cargando es un archivo, usamos este bloque
		// Primero tomamos el contenido del XML en una cadena de texto
		$content		= file_get_contents($xml);
		// Eliminamos la cabecera
		$content		= preg_replace("/^\<\?(.*?)\?\>/","",$content);
		// Quitar todos los saltos de lineas
		$content		= str_replace("\r", "", str_replace("\n", "", $content));
		// Quitar las tabulaciones
		$content		= str_replace("\t", "", $content);
		// Ahora ya podemos tomar el contenido del XML como una cadena de texto
		$d->loadXML($content);
		// Filtramos la cadena para comprobar que la estructura es valida
		$canonizadoreal = $d->C14N();
		/*
		echo $canonizadoreal;
		exit;
		*/
		// Ciframos el contenido del string
		$documentDigest	= base64_encode(hash('sha256',$canonizadoreal,true));
		// Formamos el cuerpo del cerficado digital
		...
		// Preparar la informacion de la key
		$publicPEM			= "";
		...
		$documentDigest		= base64_encode(hash('sha256',$canonizadoreal,true));
		// Preparar la informacion de la firma
		$sInfo				= '<ds:SignedInfo>'.
		...
		// Creamos el nuevo objeto de documento DOM
		$d1p				= new DOMDocument('1.0','UTF-8');
		...
		exit;
		*/
		$buscar				= '<ExtensionContent></ExtensionContent>';
		$remplazar			= '<ExtensionContent>'.$sig."</ExtensionContent>";
